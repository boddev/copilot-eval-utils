...
/** EvalGen metadata stored in schema-native m365 eval-document extensions. */
export interface EvalGenM365Extension {
  item_id?: string;
  source_location?: string;
  assertions?: Assertion[];
  category?: QuestionCategory;
  ...
  grounding_confidence?: 'high' | 'medium' | 'low';
  referenced_rows?: string[];
  synthetic_thread?: boolean;
  conversation_chaining?: boolean;
  grouping?: 'sequential_chunk';
  prompts_per_thread?: number;
  thread_id?: string;
}

/** A single m365/evalscore JSON turn generated by EvalGen. */
export interface M365EvalTurn {
  prompt: string;
  expected_response?: string;
  context?: string;
  extensions?: {
    evalgen?: EvalGenM365Extension;
  };
}

/** A multi-prompt item in the m365/evalscore JSON document. */
export interface M365EvalItem {
  name: string;
  description?: string;
  turns: M365EvalTurn[];
  extensions?: {
...
/** m365/evalscore-compatible JSON document emitted for multi-prompt evaluators. */
export interface M365EvalDocument {
  schemaVersion: '1.4.0';
...
export interface CliOptions {
  ...
  /** Emit m365/evalscore JSON with prompts grouped into multi-prompt evaluator items. */
  multiPrompt?: boolean;
  /** Number of prompts per generated multi-prompt evaluator item. */
  multiPromptTurns?: number;
  /** Optional output path for the multi-prompt JSON document. */
  multiPromptOutput?: string;
}
